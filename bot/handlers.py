...
DISCLAIMER = (
    "⚠️ Inden du starter:\n\n"
    "• Hvis botten ikke svarer med det samme efter start — vent lidt eller "
    "tryk /start igen (det sker, at botten lige genstarter).\n\n"
    "• Botten søger job og udarbejder udkast til ansøgninger — men "
    "<b>sender ikke noget selv</b>. Selve ansøgningen skal du indsende "
    "på virksomhedens hjemmeside via linket.\n\n"
    "• Filerne, botten sender (ansøgning og jobopslag), skal du selv "
    "gemme og gennemgå, og rette til efter behov, før du sender dem."
)

WELCOME = (
    "Hej! Jeg søger job på Jobindex.dk og Jobnet.dk ud fra dine søgeord.\n\n"
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📊 Bot-statistik\n\n"
        f"Unikke personer, der har startet botten: {s['total']}\n"
        f"Har uploadet CV: {s['with_cv']}\n"
        f"Har angivet søgeord: {s['with_keywords']}\n"
        f"Har angivet by: {s['with_location']}\n"
        f"Har søgt mindst én gang: {s['searched']}"
    )
...
async def _save_keywords(update: Update, telegram_id: int, text: str):
    ...
    await _reply_with_next_step(
        update, telegram_id, "Søgeord gemt: " + ", ".join(keywords)
    )
...
async def _prompt_keywords(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current:
        message = (
            "🔵 Nuværende søgeord: "
            + ", ".join(current)
            + "\n\n<b>Send nye søgeord adskilt med komma for at ændre</b>, eller tryk Annuller."
        )
    else:
        message = (
            "🔵 <b>Send søgeord adskilt med komma</b>, f.eks.:\n"
            "sælger, marketing, bogholder\n\n"
            "Søgningen går direkte til danske jobsider — brug gerne danske "
            "ord for de bedste resultater.\n\n"
            "Eller tryk Annuller, hvis du fortrød."
        )
...
async def _save_location(update: Update, telegram_id: int, text: str):
    ...
    message = (
        f"Søger i: {text}"
        if text
        else "Byfilter fjernet — søger i hele Danmark."
    )
...
async def _resolve_and_save_location(
    ...
    if canonical:
        if canonical.lower() != raw_text.strip().lower():
            await update.message.reply_text(f"Forstået som: {canonical}")
        await _save_location(update, telegram_id, canonical)
        return

    if suggestions:
        await update.message.reply_text(
            f"Fandt ikke byen «{raw_text}» blandt de officielle danske kommuner.\n\n"
            f"Mente du: {', '.join(suggestions)}?\n\n"
            "Send det rigtige navn, eller tryk Annuller for at lade det stå uændret.",
    ...
    await update.message.reply_text(
        f"«{raw_text}» findes ikke på listen over officielle danske kommuner — "
        "måske er det et lokalområde inden for en kommune (det findes også), "
        "gemmer det som det er, men søgningen finder muligvis ikke noget."
    )
...
async def _prompt_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current:
        message = (
            f"Nuværende byfilter: {current}\n\n"
            f"<b>Send et nyt bynavn</b>, tryk «{BTN_ALL_DENMARK}» "
            "(fjern filteret helt), eller «Annuller» for at lade det stå."
        )
    else:
        message = (
            "<b>Send et bynavn</b>, f.eks.: Aarhus\n\n"
            f"Jeg søger allerede i hele Danmark — «{BTN_ALL_DENMARK}» og «Annuller» "
            "gør her det samme."
        )
    await update.message.reply_text(
...
async def handle_plain_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == BTN_CANCEL:
        context.user_data.pop("awaiting", None)
        await update.message.reply_text(
            "Ok, annulleret.", reply_markup=build_keyboard(telegram_id)
        )
        return
    ...
    if text.isdigit() and awaiting is None:
        if storage.get_last_results(telegram_id):
            ...
            await update.message.reply_text(
                "Kan ikke se en jobliste (botten er måske genstartet) — "
                f"tryk {BTN_SEARCH} igen, så kan du sende jobnummeret.",
                reply_markup=build_keyboard(telegram_id),
            )
        return

    # Default: any other free-text message (including the first message
    # after tapping "Søgeord") is treated as a keywords update.
    await _save_keywords(update, telegram_id, text)
...
async def handle_cv_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not filename.lower().endswith(allowed_ext):
        await update.message.reply_text(
            "Send dit CV som PDF eller Word (.pdf, .doc, .docx)."
        )
        return
    ...
    try:
        ...
        await _reply_with_next_step(
            ...
            telegram_id,
            f"CV gemt: {filename}\n\n"
            "Kunne ikke læse teksten fra filen (til sammenligning med job) "
            "— prøv at gemme den igen som almindelig PDF eller .docx.",
        )
        return

    await _reply_with_next_step(update, telegram_id, f"CV gemt: {filename}")
...
async def cv_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and user.get("cv_path"):
        message = f"Uploadet CV: {user['cv_path'].split('/')[-1]}"
    else:
        message = "Intet CV uploadet endnu — send filen som dokument (PDF eller Word)."
    await _reply_with_next_step(update, telegram_id, message)
...
async def reset_seen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if count:
        await update.message.reply_text(f"Glemte {count} allerede viste job, søger igen...")
...
def format_vacancy(index: int, v, percent: int, detail: str, keywords: list[str]) -> str:
    ...
    if matched:
        parts.append("<b>Søgeord:</b> " + html.escape(", ".join(matched)))

    detail_escaped = html.escape(detail) if detail else ""
    parts.append(f"Match: {percent}%" + (f" — {detail_escaped}" if detail_escaped else ""))
    if v.url:
...
def _score_vacancies(telegram_id: int, vacancies: list, keywords: list[str]):
    ...
    return [
        (v, m.percent, ("ud fra ord: " + ", ".join(m.matched_keywords) if m.matched_keywords else ""))
        for v, m in scored
    ]
...
async def run_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_with_retry(
        update,
        f"Søger efter: {', '.join(keywords)}"
        + (f" i {location}" if location else " i hele Danmark")
        + " ...",
    )
    ...
    await send_with_retry(
        update,
        f"Fandt {len(to_send)}. {NUMBER_HINT_HTML}",
        parse_mode="HTML",
    )
...
async def _apply_to_vacancy_core(update: Update, context: ContextTypes.DEFAULT_TYPE, index: int):
    ...
    if not results:
        await message.reply_text(f"Start en søgning først — tryk {BTN_SEARCH}.")
        return
    if not (1 <= index <= len(results)):
        await message.reply_text(f"Job nr. {index} findes ikke — sidste liste havde {len(results)}.")
        return

    cv_text = storage.get_cv_text(telegram_id)
    if not cv_text:
        await message.reply_text(f"Kan ikke finde teksten fra dit CV — send filen igen via {BTN_CV}.")
        return
    ...
    try:
        ...
        await message.reply_text("Kunne ikke generere ansøgningen (fejl hos modellen). Prøv igen.")
        return
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            with open(vacancy_pdf_path, "rb") as f:
                await message.reply_document(
                    document=f,
                    filename=f"job_{safe_name}.pdf",
                    caption="Jobopslag som PDF — med kontaktoplysninger øverst.",
                )
        except Exception:
            logger.exception("PDF export failed for %s / %s", telegram_id, vacancy.url)
            await message.reply_text("Ansøgningen er klar, men PDF-filerne kunne ikke laves — prøv /apply igen.")
            return
...
async def apply_to_vacancy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text(
            "Angiv jobnummeret fra den seneste liste, f.eks.: /apply 3"
        )
        return
    ...
    if data == "relist":
        ...
        if not results:
            await update.effective_message.reply_text(
                f"Listen er tom — tryk {BTN_SEARCH}.",
                reply_markup=build_keyboard(telegram_id),
            )
            return
        await update.effective_message.reply_text(
