...
- reasoning: one short sentence IN DANISH (sprog — dansk) explaining the score (what matches or what is missing). Refer to the person neutrally as "kandidaten" — the applicant's gender has no bearing on the vacancy match, never guess or mention it.
...
CONSISTENCY BETWEEN percent AND reasoning — MANDATORY: if the reasoning says key domain experience/knowledge is MISSING from the CV (e.g., "mangler regnskabserfaring", "ingen erfaring med administration", "kræver en anden specialisering") — percent must be 0, even if you also mentioned some adjacent technical detail as a "plus". A medium percent (30-50%) is only acceptable when there is genuine overlap in the role's CORE tasks, and a low non-zero percent (1-20%) only when there is at least a small but literally-present relevant element in the CV (not general technical literacy).
